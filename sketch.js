...
function draw(){
  if (img) {
    //createCanvas(1000, 1000/img.width*img.height, SVG);
    ...
    getImageData(function(imageData){
      ...
      clear();
      background(10);
      sliderA.input(drawOneLineFromPixel3D);
      sliderB.input(drawOneLineFromPixel3D);
      sliderC.input(drawOneLineFromPixel3D);
...
function getMap(imageData){
  ...
  for(let i = 0; i < width; i++){
    ...
    for(let j = 0; j < height; j++){
      map[i][j] = imageData[(j * width + i) * 4] * 0.25; //1st version
